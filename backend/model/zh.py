...
class ZH(TZH):
    ...
    def get_data_by_id(table_name, id_zh):
        return DB.session.query(table_name).filter(
            table_name.id_zh == id_zh).all()
    ...
    def get_managements(self):
        q_management_structures = ZH.get_data_by_id(
            TManagementStructures, self.zh.id_zh)
        managements = []
    ...
    def get_protections(self):
        return {
            "protections": [
                ...
                for protec in [protection.id_protection for protection in ZH.get_data_by_id(CorZhProtection, self.zh.id_zh)]]
        }
    ...
    def get_eval(self):
        eval = {}
        eval.update(self.get_functions(
