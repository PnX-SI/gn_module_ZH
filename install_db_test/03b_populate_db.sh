...
psql -h 127.0.0.1 -U $user_pg -p ${db_port} -d $db_name < "${SCRIPT_DIR}/assets/db/add_pg_extensions.sql"

for table in geometry_columns geography_columns spatial_ref_sys; do
  psql -h 127.0.0.1 -U $user_pg -p ${db_port} -d $db_name -c "GRANT SELECT ON TABLE ${table} TO ${user_pg}"
done
